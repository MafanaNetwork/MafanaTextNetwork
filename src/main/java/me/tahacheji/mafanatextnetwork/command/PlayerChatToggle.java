    ...
    public void toggleChat(CommandSender sender) {
        if(sender instanceof Player) {
            ...
            if(value.equalsIgnoreCase("2")) {
                MafanaTextNetwork.getInstance().getGamePlayerMessageData().setTextValue(player, "0");
                player.sendMessage(ChatColor.GREEN + "Toggle Chat: " + ChatColor.RED + "OFF");
                //set it to 0
            } else if(value.equalsIgnoreCase("3")) {
                MafanaTextNetwork.getInstance().getGamePlayerMessageData().setTextValue(player, "1");
                player.sendMessage(ChatColor.GREEN + "Toggle Chat: " + ChatColor.RED + "OFF");
                //set to 1
            } else if (value.equalsIgnoreCase("1")){
                MafanaTextNetwork.getInstance().getGamePlayerMessageData().setTextValue(player, "3");
                player.sendMessage(ChatColor.GREEN + "Toggle Chat: " + ChatColor.GREEN + "ON");
            } else {
                MafanaTextNetwork.getInstance().getGamePlayerMessageData().setTextValue(player, "2");
                player.sendMessage(ChatColor.GREEN + "Toggle Chat: " + ChatColor.GREEN + "ON");
                //set it to 2
            }
        }
