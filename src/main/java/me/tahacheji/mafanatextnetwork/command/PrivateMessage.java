...
public class PrivateMessage {
    ...
    public void sendPrivateMessage(Player player, @Param(name = "player") OfflinePlayer target, @Param(name = "message", concated = true) String message) {
        CompletableFuture<ProxyPlayer> proxyPlayerFuture = MafanaNetworkCommunicator.getInstance().getNetworkCommunicatorDatabase().getProxyPlayerAsync(target.getUniqueId());
        proxyPlayerFuture.thenAccept(proxyPlayer -> {
            ...
            isRecipientFuture.thenAccept(isRecipient -> {
                ...
                try {
                    CompletableFuture<Void> addPrivateTextFuture = MafanaTextNetwork.getInstance().getGamePlayerMessageData().addPrivateText(player.getUniqueId(), target.getUniqueId(), message);
                    addPrivateTextFuture.thenAccept((Void) -> {
                        ...
                        player.sendMessage(ChatColor.DARK_GRAY + "[" + ChatColor.GOLD + "MTN" + ChatColor.DARK_GRAY + "] TO " + ChatColor.GOLD + target.getName() + ": " + ChatColor.WHITE + message);
                    }).exceptionally(ex -> {
                        ex.printStackTrace();
                        return null;
                    });
